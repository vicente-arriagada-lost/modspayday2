if RequiredScript == "lib/managers/hudmanager" then
	...
	function HUDManager:add_waypoint(id, data, ...)
		...
		local enable_waypoints_color = VHUDPlus:getSetting({"CustomWaypoints", "WAYPOINTS_COLOR_ENABLE"})
		...
		if waypoints_available and enable_waypoints_color then
			...
			if wp and wp.bitmap and wp.distance and wp.arrow and data.distance then
				local color = VHUDPlus:getColorSetting({"CustomWaypoints", "WAYPOINTS_COLOR"}, "white")
	...
	function HUDManager:setup_custom_waypoints()
		if managers.gameinfo and managers.waypoints then
			local bag_equip_settings = {
				["ammo_bag"] 		= VHUDPlus:getSetting({"CustomWaypoints", "SHOW_AMMO_BAG"}, true),
				["doc_bag"] 		= VHUDPlus:getSetting({"CustomWaypoints", "SHOW_DOC_BAG"}, true),
				["body_bag"] 		= VHUDPlus:getSetting({"CustomWaypoints", "SHOW_BODY_BAG"}, true),
				["grenade_crate"] 	= VHUDPlus:getSetting({"CustomWaypoints", "SHOW_GRENADE_CRATE"}, true),
				["first_aid_kit"] 	= VHUDPlus:getSetting({"CustomWaypoints", "SHOW_FIRST_AID_KIT"}, false),
			}
			...
			if VHUDPlus:getSetting({"CustomWaypoints", "SHOW_SENTRIES"}, true) then
			...
			if VHUDPlus:getSetting({"CustomWaypoints", "SHOW_ECMS"}, true) then
			...
			if VHUDPlus:getSetting({"CustomWaypoints", "SHOW_TIMERS"}, true) then
			...
			if VHUDPlus:getSetting({"CustomWaypoints", "SHOW_MINIONS"}, true) then
			...
			if VHUDPlus:getSetting({"CustomWaypoints", "LOOT", "SHOW"}, true) then
			...
			if VHUDPlus:getSetting({"CustomWaypoints", "SHOW_PAGER"}, true) then
			...
			if VHUDPlus:getSetting({"CustomWaypoints", "SHOW_SPECIAL_EQUIPMENT"}, true) then
	...
	function HUDManager:add_custom_equip_waypoint(id, unit, position, texture, texture_rect, text, color, visible_angle, visible_distance, fade_angle, offset_position)
		local params = {
			...
			scale = 1.25 * VHUDPlus:getSetting({"CustomWaypoints", "WAYPOINTS_SCALE"}, 1),
			icon = {
				type = "icon",
				show = true,
				texture = texture,
			...
			if data.jammer_active or data.feedback_active then
				local params = {
					...
					icon = {
						type = "icon",
						show = true,
						scale = 1.1 * VHUDPlus:getSetting({"CustomWaypoints", "WAYPOINTS_SCALE"}, 1),
					...
					distance = {
						type = "distance",
						show = true,
						scale = 0.6 * VHUDPlus:getSetting({"CustomWaypoints", "WAYPOINTS_SCALE"}, 1),
					},
		...
		if event == "set_active" then
			...
			if data.active and not timer_data.ignore then
				...
				local params = {
					...
					scale = 1.35 * VHUDPlus:getSetting({"CustomWaypoints", "WAYPOINTS_SCALE"}, 1),
					icon = {
						type = "icon",
						show = true,
						show_offscreen = true,
					...
					timer = {
						...
						initial_value = 0,
						font = "fonts/font_medium_shadow_mf",
						scale = 1.50 * VHUDPlus:getSetting({"CustomWaypoints", "WAYPOINTS_SCALE"}, 1),
					},
					speed_upgrade = {
						type = "icon",
						show = false,
						...
						color = Color.white,
						scale = 0.5 * VHUDPlus:getSetting({"CustomWaypoints", "WAYPOINTS_SCALE"}, 1),
		...
		if event == "add" then
			...
			local params = {
				...
				health_bar = {
					...
					scale = 1.65 * VHUDPlus:getSetting({"CustomWaypoints", "WAYPOINTS_SCALE"}, 1),
				...
				kills = {
					...
					color = Color.white,
					alpha = 0.8,
					scale = 0.7 * VHUDPlus:getSetting({"CustomWaypoints", "WAYPOINTS_SCALE"}, 1),
				},
	...
	function HUDManager:custom_waypoint_loot_clbk(event, key, data)
		...
		if event == "add" then
			if tweak_entry and not tweak_entry.is_vehicle and not tweak_entry.skip_exit_secure and (data.carry_id ~= "person" or managers.job:current_level_id() == "mad" and (data.bagged or data.unit:editor_id() ~= -1)) then
				local angle = HUDManager.CUSTOM_WAYPOINTS.DEBUGGING and 180 or VHUDPlus:getSetting({"CustomWaypoints", "LOOT", "ANGLE"}, 25)
				...
				if bag_name then
					local params = {
						unit = data.unit,
						offset = data.bagged and Vector3(0, 0, VHUDPlus:getSetting({"CustomWaypoints", "LOOT", "BAGGED_OFFSET"}, 30)) or Vector3(0, 0, VHUDPlus:getSetting({"CustomWaypoints", "LOOT", "OFFSET"}, 15)),
						...
						icon = {
							type = "icon",
							show = data.bagged and VHUDPlus:getSetting({"CustomWaypoints", "LOOT", "ICON"}, true),
							std_wp = "wp_bag",
							alpha = 0.5,
							on_minimap = true,
						},
			...
			if data.active then
				local params = {
					...
					icon = {
						...
						texture_rect = { 1 * 64, 4 * 64, 64, 64 },
						scale = 1.4 * VHUDPlus:getSetting({"CustomWaypoints", "WAYPOINTS_SCALE"}, 1),
						on_minimap = true,
					},
					duration = {
						type = "duration",
		...
		if event == "add" then
			...
			if icon_data then
				local params = {
					...
					scale = 1 * VHUDPlus:getSetting({"CustomWaypoints", "WAYPOINTS_SCALE"}, 1),
					alpha = HUDManager.CUSTOM_WAYPOINTS.DEBUGGING and 1 or 0.1,
...
if RequiredScript == "lib/units/vehicles/vehicledrivingext" then
	...
	function VehicleDrivingExt:add_loot(...)
		...
		if managers.hud and self._unit:unit_data() and not self._unit:unit_data().name_label_id and not MUIMenu then
