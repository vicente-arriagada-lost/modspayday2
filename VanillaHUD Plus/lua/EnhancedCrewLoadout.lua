...
if string.lower(RequiredScript) == "lib/managers/menu/contractboxgui" then
	...
	function ContractBoxGui:init(...)
		...
		if managers.network:session() then
			...
			self:set_loadout_enabled(VHUDPlus:getSetting({"CrewLoadout", "SHOW_IN_LOBBY"}, true) and managers.menu:is_pc_controller())
		end
	end
	...
	function ContractBoxGui:set_enabled(...)
		...
		self:set_loadout_enabled(self._enabled and VHUDPlus:getSetting({"CrewLoadout", "SHOW_IN_LOBBY"}, true) and managers.menu:is_pc_controller())
	end
		...
		for i, peer_id in ipairs({3, 2, 1, 4}) do
			if not self._peer_loadout[peer_id] then
				self._peer_loadout[peer_id] = LoadoutPanel:new(self._loadout_data, self, peer_id, width, ContractBoxGui._LOADOUT_H + 8, {
					component_layout = VHUDPlus:getTweakEntry("STD_LOBBY_LOADOUT_LAYOUT", "table",
						{
							{ "playtime", "ping" },
							{ "name" },
							{ "character" },
...
elseif string.lower(RequiredScript) == "lib/managers/menu/crimespreedetailsmenucomponent" then
	...
	function CrimeSpreeDetailsMenuComponent:populate_tabs_data(tabs_data, ...)
		...
		if not self:_is_in_preplanning() and not Global.game_settings.single_player and VHUDPlus:getSetting({"CrewLoadout", "SHOW_IN_CS_LOBBY"}, true) then
	...
	function CrimeSpreeDetailsMenuComponent:update(t, dt, ...)
		...
		if VHUDPlus:getSetting({"CrewLoadout", "SHOW_IN_CS_LOBBY"}, true) then
		...
		for i, peer_id in ipairs({3, 2, 1, 4}) do
			if not self._peer_loadout[peer_id] then
				self._peer_loadout[peer_id] = LoadoutPanel:new(self._loadout_data, self, peer_id, width, math.floor(self._loadout_data:h() + 9), {
					component_layout = VHUDPlus:getTweakEntry("CS_LOBBY_LOADOUT_LAYOUT", "table",
						{
							{ "playtime", "ping" },
							{ "name" },
							{ "skills" },
	...
	function TeamLoadoutCustom:init(panel, text, i)
		...
		for peer_id = 1, 4 do
			self._player_slots[peer_id] = LoadoutPanel:new(self._panel, self, peer_id, quarter_width, self._panel:h() + 7, {
				component_layout = VHUDPlus:getTweakEntry("BRIEFING_LOADOUT_LAYOUT", "table",
					{
						{ "perk" },
						{ "skills" },
						{ "primary" },
	...
	for id, ptr in pairs(TeamLoadoutItem) do
		if type(ptr) == "function" then
			TeamLoadoutCustom[id] = TeamLoadoutCustom[id] or function(self, ...)
				VHUDPlus:print_log(string.format("TeamLoadoutCustom: Unoverwritten function call: %s", id), "warning")
			end
		end
	end
	if CoreClass and VHUDPlus:getSetting({"CrewLoadout", "REPLACE_IN_BRIEFING"}, true) then
...
elseif string.lower(RequiredScript) == "lib/managers/hud/newhudstatsscreen" then
	...
	function HUDStatsScreen:_create_player_info()
		self._USING_CREW_LOADOUT = VHUDPlus:getSetting({"CrewLoadout", "SHOW_ON_STATS_PANEL"}, true) and not (_G.LobbyPlayerInfo and LobbyPlayerInfo.settings.show_skills_in_stats_screen)
			...
			for peer_id = 1, 4  do
				if not self._peer_loadout[peer_id] then
					self._peer_loadout[peer_id] = LoadoutPanel:new(parent_panel, self, peer_id, parent_panel:w(), parent_panel:h() * 0.17, {
						component_layout = VHUDPlus:getTweakEntry("TAB_LOADOUT_LAYOUT", "table",
							{
								{ "name", "ping" },
								{ "skills", "perk" },
							}),
	...
	function HUDStatsScreen:arrange_loadout_panels(parent_panel)
		if parent_panel then
			...
			parent_panel:set_leftbottom(10, self._right:h() - (VHUDPlus:getSetting({"TabStats", "ENABLED"}, true) and 10 or 40))
		end
	end
end