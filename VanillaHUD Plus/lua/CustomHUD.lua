...
-- if VHUDPlus:getSetting({"CustomHUD", "WolfENABLED"}, false) and not VHUDPlus:getSetting({"CustomHUD", "ENABLED"}, true) then
	...
	--[[
		...
		function HUDTeammateCustom:set_weapon(index, id, skin_id, silencer)
			self:call_listeners("weapon", index, id, skin_id, silencer)
		end
		...
		function PlayerInfoComponent.Base.get_item_icon_data(type, id, weapon_id)
			local tweak_entry = {
			...
			local bundle_folder, guis_catalog

			if type == "weapon_skin" then
				bundle_folder = skin.texture_bundle_folder
				guis_catalog = "guis/dlcs/cash/safes/" .. tostring(bundle_folder) .. "/"

				if skin then
					local base_weapon_id = skin.weapon_id
					local equipped_weapon_id = weapon_id or base_weapon_id
					local texture_name

					if skin.weapon_ids and equipped_weapon_id and table.contains(skin.weapon_ids, equipped_weapon_id) and equipped_weapon_id ~= base_weapon_id then
						texture_name = tostring(id) .. "_" .. tostring(equipped_weapon_id)
					else
						texture_name = skin.texture_name or tostring(id)
					end

					local texture = string.format("%s%s%s", guis_catalog, texture_path.weapon_skin, texture_name)

					local weapon_entry = tweak_data.weapon[equipped_weapon_id or base_weapon_id]
					local name_id      = weapon_entry and weapon_entry.name_id or tostring(equipped_weapon_id or base_weapon_id)
			...
			bundle_folder = tweak_entry[type][id] and tweak_entry[type][id].texture_bundle_folder
	...
	--[[
		...
		function PlayerInfoComponent.Weapon:set_weapon(weapon_id, skin_id, silencer)
			local has_skin = skin_id and tweak_data.blackmarket.weapon_skins[skin_id]
			local bitmap_texture, text = PlayerInfoComponent.Base.get_item_icon_data(has_skin and "weapon_skin" or "weapon", has_skin and skin_id or weapon_id, weapon_id)
		...
		function PlayerInfoComponent.SpecialEquipment:arrange()
			...
			for i, panel in ipairs(self._special_equipment) do
				...
				panel:set_left(0 + column * panel:w())
	...
	if RequiredScript == "lib/managers/hudmanagerpd2" then
		...
		function HUDManager:add_weapon(data, ...)
			...
			if wbase:get_cosmetics_id() ~= nil then
				skin_id = wbase:get_cosmetics_id()
			...
			self:set_teammate_weapon(HUDManager.PLAYER_PANEL, data.inventory_index, weapon_id, skin_id, wbase:got_silencer())
		...
		function HUDManager:set_teammate_weapon(i, index, weapon_id, skin_id, silencer)
			self._teammate_panels[i]:set_weapon(index, weapon_id, skin_id, silencer)
		end
			...
			if peer_id == local_peer:id() then
				...
				if outfit then
					...
					for selection, data in ipairs({ outfit.secondary, outfit.primary }) do
						...
						self:set_teammate_weapon(panel_id, selection, weapon_id, weapon_skin and skin_id or nil, silencer)
