if RequiredScript == "lib/units/enemies/cop/copdamage" and not VHUDPlus.fix2 then
	VHUDPlus.fix2 = true
	...
	function CopDamage:_process_popup_damage(data)
		CopDamage.DMG_POPUP_SETTING = VHUDPlus:getSetting({"DamagePopupNew", "DISPLAY_MODE"}, 2)
	...
	function CopDamage:show_popup(damage, dead, headshot, critical, color_id)
		if managers.waypoints then
			...
			local waypoint_color = color_id and ((color_id == 5 and VHUDPlus:getSetting({"CustomHUD", "TEAMMATE", "AI_COLOR", "USE"}, false)) and VHUDPlus:getColorSetting({"CustomHUD", "TEAMMATE", "AI_COLOR", "COLOR"}, Color.white) or tweak_data.chat_colors[color_id]) or VHUDPlus:getColorSetting({"DamagePopupNew", critical and "CRITICAL_COLOR" or headshot and "HEADSHOT_COLOR" or "COLOR"}, "yellow")
			waypoint_color = waypoint_color:with_alpha(VHUDPlus:getSetting({"DamagePopupNew", "ALPHA"}, 1))
			local waypoint_duration = VHUDPlus:getSetting({"DamagePopupNew", "DURATION"}, 3)
			if waypoint and not waypoint:is_deleted() then
				...
				local params = {
					unit = self._unit,
					offset = Vector3(10, 10, VHUDPlus:getSetting({"DamagePopupNew", "HEIGHT"}, 20)),
					scale = 2 * VHUDPlus:getSetting({"DamagePopupNew", "SCALE_ALT"}, 1),
					...
					icon = {
						type = "icon",
						show = dead,
						scale = VHUDPlus:getSetting({"DamagePopupNew", "SKULL_SCALE"}, 1.2),
					...
					component_order = { VHUDPlus:getSetting({"DamagePopupNew", "SKULL_ALIGN"}, 1) == 1 and { "icon", "label" } or { "label", "icon" } , { "duration" } }
				}
